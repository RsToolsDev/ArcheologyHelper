function PORTER_COST_CATEGORY_MAPPINGS() {
 ...
 var porters = [
    { "tier":"I (6)", "charges":"5", "energy":"pale energy", "energyAmount":30, "jewelry":"sapphire necklace"},
    { "tier":"II (28)", "charges":"10", "energy":"bright energy", "energyAmount":35, "jewelry":"sapphire necklace"},
    { "tier":"III (48)", "charges":"15", "energy":"sparkling energy", "energyAmount":40, "jewelry":"emerald necklace"},
    { "tier":"IV (68)", "charges":"20", "energy":"vibrant energy", "energyAmount":45, "jewelry":"emerald necklace"},
    { "tier":"V (88)", "charges":"25", "energy":"radiant energy", "energyAmount":60, "jewelry":"ruby necklace"},
    { "tier":"VI (99)", "charges":"30", "energy":"incandescent energy", "energyAmount":80, "jewelry":"diamond necklace"}
]
...
async function getPriceDependencies(){
    ...
    porters.forEach((porter) => {
        ...
        tableData += addRowInHTML(addColumnInHTML(porter.tier, 'col-1 my-2') 
            + addColumnInHTML(addImageInHTML(porter.energyIcon) + " " 
                + porter.energyAmount + " x " 
                + porter.energy.replace(' energy', '')
                + " (" + priceOfEnergy + " gp) ", 'col-4 my-2', "text-left")
            + addColumnInHTML(addImageInHTML(porter.jewelryIcon) + " " 
                + porter.jewelry.replace(' necklace', '')
                + " (" + porter.jewelryCost + " gp) " , 'col-3 my-2', 'text-left')
            + addColumnInHTML(porter.charges + " (" + pricePerCharge + " gp) " , 'col-2 my-2')
            + addColumnInHTML(pricePerCharge * .5 + "K", 'col-2 my-2'));
    });
